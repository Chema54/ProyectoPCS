...
import main.negocio.dto.ExperienciaPracticanteDTO;
...
public class ExperienciaPracticanteDAO extends MoldeDAOCompleto<ExperienciaPracticanteDTO, String> {

    private static final Logger LOGGER = LogManager.getLogger(ExperienciaPracticanteDAO.class);
    ...
    @Override
    public void createOne(ExperienciaPracticanteDTO dto) throws ExcepcionMostrableUsuario {
        try (
    ...
    @Override
    public List<ExperienciaPracticanteDTO> getAll() throws ExcepcionMostrableUsuario {
        try (
            Connection connection = ConexionBD.getInstance().getConnection();
            PreparedStatement statement = connection.prepareStatement(GET_ALL_QUERY);
            ResultSet resultSet = statement.executeQuery()
        ) {
            List<ExperienciaPracticanteDTO> list = new ArrayList<>();
            while (resultSet.next()) {
                list.add(mapResultSetToDTO(resultSet));
            }
            return list;
        } catch (SQLException e) {
            throw ManejadorExcepciones.handleSQLException(LOGGER, e, "No ha sido posible cargar las relaciones de practicante y experiencia.");
        }
    }

    @Override
    public ExperienciaPracticanteDTO getOne(String idCompuesto) throws ExcepcionMostrableUsuario {
        throw new UnsupportedOperationException("Método getOne no soportado para llaves compuestas en este DAO.");
    }

    @Override
    public void updateOne(ExperienciaPracticanteDTO dto) throws ExcepcionMostrableUsuario {
    ...
    public void deleteOne(String idCompuesto) throws ExcepcionMostrableUsuario {
        ...
        try (
            ...
            throw ManejadorExcepciones.handleSQLException(LOGGER, e, "No ha sido posible eliminar la relación de practicante y experiencia.");
        }
    }
